...
<div class='row'>
  ...
  <div class='col-sm-12 course-section'>
    ...
    <!--<div class='section-title' data-release-date="" data-days-until-dripped="0" data-is-dripped-by-date="" data-course-id="156515">
     ...
     <table style="width:95%;margin-left:20px;" border="1" cellspacing="1" cellpadding="1" align="center" id="writ">
                         <tr style="background-color:#226e99;color:#fff;text-align:center;"><th>Sl No</th><th>Subject</th><th>Title</th><th>Publish</th><th>Download</th><th>Upload</th><th>Status</th><th>Corrected Answer</th><th>Mark</th><th>Answer Key</th></tr>
                                                                                 @foreach($data as $object)

                                                        {{ csrf_field() }}
                                                        <?php
														
			         $ws = DB::table('tbl_writings')->where(['student_id' =>$userid])->where('writing_id', '=', $object->wid)->get(); 

					 if(count($ws)==0){$status='Not Uploaded';} 
					 ...
					 else if($ws[0]->wstatus=='2')
					 {
					 $status='Rejected';
					 }
					 else if($ws[0]->wstatus=='0')
					 {
					 $status='Uploaded';
					 }
					 else if($ws[0]->wstatus=='3')
					 {
					 $status='ReUploaded';
					 }
					 }
					 $today=date('Y-m-d');
					 $d_vfrom1 = date('Y-m-d', strtotime($object->dvalidfrom));
					 $d_vto1 = date('Y-m-d', strtotime($object->dvalidto));
					 if($today>=$d_vfrom1 && $today<=$d_vto1)
					 {
					 $download=1;
					 }
					 ...
					 $u_vfrom1 = date('Y-m-d', strtotime($object->uvalidfrom));
				 $u_vto1 = date('Y-m-d', strtotime($object->uvalidto));
					//echo $today;
					 if($today>=$u_vfrom1 && $today<=$u_vto1)
					 {
					 ...
					 if($today==date('Y-m-d', strtotime($object->adop)))
					 {
					 $answer=1;
					}
					else
					...
					$answer=0;
														...
														?>
                                                        <?php if($i%2==0) { $style='style=background-color:#e8e8e8;'; } else { $style='style=background-color:#d7e3e9;';}?>
                                                         <tr {{$style}}><td>{{ $i }}</td>
                                                         <td>{{$object->subject}}</td>
                                                         <td>{{$object->title}}</td>
                                                        <td> {{$object->dop}}</td>
                                                        <td> From {{$object->dvalidfrom}} to {{$object->dvalidto}}&nbsp;<?php if($download=='1') { ?><a href="{{ url('/'.$object->question_pdf) }}" target="_blank"><i class="fa fa-download"></i></a><?php } ?></td>
                                                        <td> From {{$object->uvalidfrom}} to {{$object->uvalidto}}&nbsp;<?php if($upload=='1' && ($status=='Not Uploaded' || $status=='Rejected')) { ?><a href="{{ url('/writinganswer-upload/'.$object->wid.'/'.$pgmtime) }}"><i class="fa fa-upload"></i></a><?php } ?></td>
                                                        <td>{{$status}}</td>
                                                        <td><?php if(count($ws)!=0){ if($ws[0]->fanswer_pdf!='') { ?><a href="{{ url('/'.$ws[0]->fanswer_pdf) }}" target="_blank"><i class="fa fa-download"></i></a><?php }} ?></td>
                                                        <td>@if(count($ws)!=0) {{$ws[0]->mark}} @endif</td>
                                                        <td>On {{$object->adop}}</td>
                                                        </tr>
                                                         <?php $i++;?>
@endforeach
                         </table>
